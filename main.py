...
class MapParams:
    ...
    def update(self, event):
        print(event.key)
        if event.key == pygame.K_UP and self.zoom < 19:
            self.zoom += 1
        elif event.key == pygame.K_DOWN and self.zoom > 2:
            self.zoom -= 1
        elif event.key == pygame.K_a:
            self.lon -= LON_STEP * math.pow(2, 13 - self.zoom)
        elif event.key == pygame.K_d:
            self.lon += LON_STEP * math.pow(2, 13 - self.zoom)
        elif event.key == pygame.K_w and self.lat < 85:
            self.lat += LAT_STEP * math.pow(2, 13 - self.zoom)
        elif event.key == pygame.K_s and self.lat > -85:
            self.lat -= LAT_STEP * math.pow(2, 13 - self.zoom)
        elif event.key == 49:
            self.type = "map"
        elif event.key == 50:
